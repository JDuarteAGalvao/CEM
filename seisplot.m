...
%
% MODIFIED BY LA - 03 OCT 2012   
%

if nargin<=7, sf=0.5; end; %default scale factor
if nargin<=6, ss=[]; end;  %default scale factor
if nargin<=5, nd=0; end;  %default nd
if nargin<=3, g0=0; dg=1; end;  %default g0, dg
if nargin==1, t0=0; dt=1; g0=1; dg=1; nd=0; end;
...
zw0=zeros(nr+2,size(y,2)); zw0(2:nr+1,:)=z;
...
%  y(:,i)=y(:,i)+(i-1.)*0.5; 
%  zw0(:,i)=zw0(:,i)+(i-1.)*0.5;
  y=multscale*y; zw0=multscale*zw0;
...
plot(y,ty,'-r'), hold off
set(gca,'ydir','reverse');
axis([0 max(zw0(:,end)) tz(1) tz(end)]);
ylabel('Time (ms)', 'FontSize',14); xlabel('Range index', 'FontSize',14);
xticks([1 2 3])
